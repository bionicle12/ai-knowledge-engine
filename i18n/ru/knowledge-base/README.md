...
|---------|------------|-----------|--------------------|
| `!view` | Запускает или повторно открывает локальный read-only граф знаний | 0 токенов | Чтобы без AI просматривать страницы, связи, метаданные и диагностику |
...
- **`!view`** — когда нужно посмотреть, что уже находится в `knowledge/`; `!view status` показывает URL, а `!view stop` останавливает локальный сервер
