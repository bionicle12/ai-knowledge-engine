...
Пиши session summary по `!save` или когда пользователь явно просит
сохранить (или «зафиксируй в базе»). Не определяй сам, что «накопилось
достаточно материала», и не пиши от себя.
...
| Пользователь явно просит сохранить | То же, что `!save` |
...
## Assumptions
- Положил `q3-notes.md` в `domain/`, а не в `decisions/` — в тексте только обсуждение
- «DF» в стенограмме = DragonflyDB (три соседних абзаца)
- importance=7 по объёму синтеза; пользователь не подтверждал
```

На пограничном случае бери самый осторожный слот, запиши одну строку
допущения и продолжай. Спрашивай только если от ответа зависит итог.
Lint info, если на одну `knowledge/<area>/` больше 3 допущений за 30 дней
→ уточни `DATA_PLACEMENT_EXAMPLES.md` для этой области.
...
reflection:
  ...
  max_insights_per_run: 3        # потолок, не квота — ноль инсайтов нормален
```

### Тормоза

- **Честный ноль.** Нет нового инсайта — валидный ожидаемый исход. Запиши
  (`python3 scripts/kb_reflect.py --record-result 0`) и остановись. Не
  выдумывай инсайты, чтобы заполнить `max_insights_per_run`.
- **Мета-инсайт (уровень 2)** только если с прошлой рефлексии уровня 1
  появились новые факты уровня 0 (ingest). `kb_reflect.py` ставит
  `meta_insight_allowed`.
- **Застой.** Два нулевых прогона подряд → по расписанию
  (`--check-threshold`) остаётся `SKIP`. Команда `!reflect` от человека
  всё ещё работает.
...
5. Не больше `max_insights_per_run` страниц в `knowledge/insights/` со
   ссылками `[[wikilinks]]` — или ни одной и `--record-result 0`
...
## `!audit` — чистый контекст, по пакам

Писавший страницу не должен быть единственным проверяющим. `kb_reindex.py`
пишет `.repomix/audit/<pack>__request.md` и `CROSS_PACK__request.md`
(только заголовки).

`!audit` → **новая сессия** на каждый request. Находка без `file:line` и
цитаты не идёт в отчёт. Скрипта `kb_audit.py` нет.

## `!profile-review`

`knowledge/profile/` с `lifecycle: permanent` — L1 stale lint его
пропускает.

1. Прочитай, что ты знаешь об авторе из `knowledge/profile/`. Чего не
   хватает? Спрашивай **по три вопроса**, с важных. По ответам обновляй
   страницы.
2. Затем: что может быть устаревшим или неполным? Тот же ритм.

Поставь `profile_review.reviewed_at` в `kb.config.yml`. Lint предупреждает
через 30 дней, если поле есть; нет поля — нет предупреждения.

## `!quiz`

База экзаменует владельца. Это единственная команда, которая проверяет
человека, а не страницы.

1. Просмотри `knowledge/profile/`, `knowledge/principles/`,
   `knowledge/decisions/` и `knowledge/routing-table.md`. Не грузи всю базу.
2. Задай **пять вопросов** о том, что уже лежит в базе. Начни с
   **самых дорогих ошибок** (тихое противоречие, решение, которое
   владелец не хочет увидеть перевёрнутым, факт, который AI выдумает,
   если страница тонкая). Потом дыры покрытия. Потом проверка
   датированного решения.
3. По одному вопросу — нормально; пачкой — только если попросили.
4. Лист ответов — `interactions/quiz/YYYY-MM-DD.md` (не индексируется):
   вопрос, ответ владельца, страница, которая должна была это держать,
   нужна ли правка. Операция в логе: `quiz`.
5. Если ответ противоречит странице — не «чини» молча. Покажи diff и жди.

Скрипта `kb_quiz.py` нет.

## Слот исследования (`!reflect`)

Если `kb_reflect.py --json` (или `--generate`) вернул пары в
`exploration`, возьми **не больше двух** как вопросы рефлексии: «Что
связывает [[A]] и [[B]], чего граф не показывает?» Пары — разные
компоненты (или узлы без направленного пути) с пересечением тегов/заголовков
`< 0.8`. Пустой список валиден — не выдумывай связь. Новые выводы — в
`knowledge/insights/`.
...
- `quiz` — когда `!quiz` пишет `interactions/quiz/`
