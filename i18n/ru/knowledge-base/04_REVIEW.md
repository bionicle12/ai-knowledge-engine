...
Пользователь может написать `!review` в чате, чтобы агент разобрал очередь. Контракт:

1. **Сканировать в этом порядке** (сначала самое значимое):
   - `review/needs-redaction/` — чувствительные материалы; либо отредактировать и перенаправить, либо архивировать в `excluded-sensitive/`
   - `review/needs-ai-decision/` — основная работа
   - `review/needs-classification/` — неопределённый тип
2. **По каждому элементу** агент отчитывается:
   - **Исходный файл**: какой review-пакет (путь)
   - **Решение**: `extract` / `redact` / `archive` / `defer-to-user`
   - **Цели**: какие файлы `knowledge/<категория>/<slug>.md` созданы или обновлены
   - **Почему**: 1-2 предложения обоснования
3. **Защита от длинных книг**: если review-пакет содержит блок «⚠️ Likely long-form reference book» (добавляется `kb_ingest.py` для PDF/EPUB/DOCX объёмом ≥25k слов), агент **обязан**:
   - НЕ копировать прозу в `knowledge/voice/`
   - Написать заметку с выводами в `knowledge/principles/<book-slug>-takeaways.md` (5–15 буллетов, словами пользователя)
   - Обновить `knowledge/principles/<role>-bookshelf.md` (создать, если нет)
   - Сослаться на путь к ассету через frontmatter `source:`
4. **Defer-to-user**, когда требуется участие пользователя — задать **один конкретный вопрос на элемент**, собрав их пакетом в конце. Никогда не блокировать всю очередь в ожидании ответа.
5. **Удалить review-пакет** после обработки. Добавить запись `review` в `log.md`.
6. **Reindex** в конце (или напомнить пользователю).

Если очередь пуста, ответить: *«Очередь review пуста — обрабатывать нечего.»*

Если очередь огромная (>20 элементов) — обработать первые 5–10 и спросить пользователя, продолжать ли.
