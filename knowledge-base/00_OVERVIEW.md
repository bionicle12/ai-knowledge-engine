...
The agent runs `bash setup/shell/install.sh` after orientation. This flattens the payload directly into the project root — no nested `knowledge-base/` folder, no extra `cd` to work with the base.
...
├── DATA_PLACEMENT_EXAMPLES.md         ← generated by kb_populate.py from examples/<role>.yml
├── START_HERE.md                      ← parameterized from templates/START_HERE.md.template (post-deploy)
...
│   ├── reindex.sh, lint.sh, doctor.sh, watcher.sh
│   ├── reindex.command, watcher-start.command, watcher-stop.command   (macOS)
│   └── reindex.bat, watcher-start.bat                                  (Windows)
│
├── templates/                         ← Kept on disk for re-runs (kb_populate, kb_upgrade)
├── examples/                          ← Role YAMLs (kept for !populate command)
│
