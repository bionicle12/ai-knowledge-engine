...
> **Deployment helper:** `shell/install.sh` flattens `setup/` into the project root (recommended approach below).

---

## Deployment flow (where things end up)

The user copies the upstream `knowledge-base/` folder into their project root, **renamed to `setup/`** so it doesn't clash with file names the agent will create. The expected layout right after the copy:
...
The agent reads `setup/00_OVERVIEW.md` first, then walks through the modules. When ready to materialize the base, it runs:

```bash
bash setup/shell/install.sh
```

This script:
1. Moves `scripts/`, `shell/`, `templates/`, `examples/` and the `.md` modules from `setup/` into the project root
2. Removes the now-empty `setup/` folder
3. Calls `kb_ingest.py --init-dirs` to create `raw/`, `processed/`, `knowledge/`, `assets/`, `assets-index/`, `review/`, `interactions/`
4. Marks `*.sh` and `*.command` files executable

**Result:** everything lives in the project root — no nested `knowledge-base/`, no extra terminal navigation:

```
my-project/
├── kb.config.yml                    ← parameterized by the agent
├── AGENTS.md                        ← parameterized by the agent
├── KNOWLEDGE_STRUCTURE.md
├── DATA_PLACEMENT_EXAMPLES.md       ← generated by kb_populate.py
├── START_HERE.md                    ← post-deploy summary for the user
├── repomix.config.json
├── requirements.txt
...
├── shell/      (POSIX wrappers)
├── templates/  (kept for re-population & upgrades)
├── examples/   (role YAMLs)
...
If the user already chose a different layout (e.g., they want the base in a subfolder), the agent should respect it — but the default and recommended path is **flat at the project root**.
...
7. Show the user a 3-5 line summary in chat that:
